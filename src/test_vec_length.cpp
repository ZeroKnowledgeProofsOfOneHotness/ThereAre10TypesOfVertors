...
int main(int argc, char** argv)
{
    ...
    while(size<=SIZE){
        ...
        for(int i=0;i<TRIALS;i++)
        {
            #if defined(THISWORK)
            {
                ZKP_Prover_Vector_ThisWork prover(P,vector<Pedersen_Commitment>(Commitments.begin(),Commitments.begin()+size),LOC);
                ZKP_Verifier_Vector_ThisWork verifier(P,vector<Pedersen_Commitment>(Commitments.begin(),Commitments.begin()+size),255);
        ...
        {
            #if defined(THISWORK)
            cout<<THISWORK_PROVER_TAGS[i]<<","<<size;
        ...
        {
            #if defined(THISWORK)
            cout<<THISWORK_VERIFIER_TAGS[i]<<","<<size;
            for(int j=0;j<TRIALS;j++)
            {
